...
| B20b `factory_snmp` | `python3 /opt/pktr/scripts/snmp_attack.py walk` then `flag`, then `stop` | `public` dumps the `.1.3.6.1.4.1.53864.1` subtree; the flag is the `opsNote` OID; `snmpset` with `private` stops the line on the map; submit → 150 base |
