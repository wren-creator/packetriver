...
`simmap` reads it and moves the train. Throw the switch to the spur before the
train reaches the branch (under the "R" of "Main Rail Line" on the map) and the
train routes onto the spur, runs it down to the Widget Factory, and piles in.
The factory catches fire.
...
| Physical result | `simmap` mirrors the switch position. With it set to `spur` when the train crosses the branch, `rail.on_spur` latches and the train travels the spur polyline over ~5 s; on arrival `rail.derailed` and `factory.on_fire` set — the train parks at the plant and a fire ring shows on the Widget Factory. |
