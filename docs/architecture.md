...
| Service | Stack | Role |
|---|---|---|
| `gateway` | nginx:alpine | The only way in. Proxies `/`, `/api/state`, `/ws` → `simmap` and `/api/score/*` → `scoring`. Also does `Host:`-header vhost routing: each `<shop>.packetriver.range` (CNAME'd to the gateway) is routed to its subdir of the shared `websites` container with a distinct faked `Server` / `X-Powered-By` per `map $host ...`. Writes a JSON access log to the `gw-logs` volume for the Alert-Level tailer. |
...
| `factory-snmp` | alpine + net-snmp | The Widget Works line-management box. A real SNMP agent with the shipped communities (`public` ro, `private` rw). `public` reads the line-management subtree (`.1.3.6.1.4.1.53864.1`, one OID is the `factory_snmp` flag); an `snmpset` to the line-enable OID publishes `pkt/factory/cmd` and stops the line. `SNMP_HARDENED=1` in the segmented build. |
