...
town (see [`learning-design.md`](learning-design.md)). So each entry here gives
the mechanism, the real-world parallel, the MITRE mapping, the tool family, the
surface, and the physical consequence, and points at
[`../instructor/answer-key.md`](../instructor/answer-key.md) for the exact
sign-on / payload / flag location. The district docs under
[`districts/`](districts/) carry the same Tier-1 detail for players.
...
**Confirm / exploit with:** <tool family + surface; exact steps in the answer key>
...
**Fix:** parameterised queries; turn verbose errors off in production.
...
**Fix:** enforce ownership server-side on every record read.
...
**Fix:** prepared statements and a real credential check.
...
**Points / severity:** 100 / quiet.
**Fix:** keep backups out of the docroot; block archive/SQL extensions at the server.
...
**Fix:** output-encode on render; add a CSP; treat back-office views as untrusted-input surfaces.
...
**Points / severity:** 125 / medium.
**Fix:** force a credential change on first use; issue real session tokens.
...
**Points / severity:** 100 / quiet.
**Fix:** don't deploy the repo; block dotfiles at the server; rotate anything that was ever committed.
...
**Points / severity:** 150 / medium.
**Fix:** allowlist egress, block loopback / link-local / internal ranges, and don't return the raw fetch body.
...
**Fix:** real auth on the clerk's office; output-encode the notice body; CSP.
...
**Fix:** canonicalise and confine the resolved path under a base directory; reject separators in `doc`.
...
**Fix:** keep operational config out of the docroot; block config paths at the server.
...
**Fix:** force a change at install; no shared defaults.
...
**Fix:** authorise every receipt read; use non-sequential public handles. (Card data here is vendor **test** PANs only, non-functional.)
...
**Fix:** `QSECURITY` 40+, require passwords, rotate shipped defaults, `QLMTSECOFR`, and pull `*PUBLIC` on the payroll library down to `*EXCLUDE` with an authorization list.
...
**Points / severity:** 200 / loud.
**Fix:** pin the algorithm (HS256 only), reject `none`, verify the signature; don't render a settlement token into HTML.
...
**Points / severity:** 125 / medium.
**Fix:** enforce ownership on every account read; keep secrets out of free-text fields.
...
**Points / severity:** 175 / loud.
**Fix:** segment OT so nothing routable reaches 502; source-allowlist to the HMI/historian; move to an authenticated protocol. `./start.sh --segmented` sets `MODBUS_WRITE_OPEN=0` and the PLC re-asserts golden every scan.
...
**Points / severity:** 175 / loud.
**Fix:** as water, segment, source-allowlist, authenticate.
...
**Points / severity:** 175 / loud.
**Fix:** as above; keep the safety function on a separate, non-writable system.
...
**Points / severity:** 175 / loud.
**Fix:** broker ACLs + auth (the segmented build's `acl.segmented` + `passwd.segmented`), signed commands, keep the bus off any routable path.
...
**Points / severity:** 200 / loud.
**Fix:** drop the raw console, or put it behind auth + a source allowlist; never shell out on operator input.
